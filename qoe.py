...
def send_message(image_path, client,info):
    fixed_msg_1 = f"""
    I have uploaded {len(image_path)} frames, each representing a video chunk of 2 seconds. You first extract the frame number attached below the image content. The original video title is {info}. Your task is as below:
    1. Based on the title background, first summarize some keywords that may attract viewers. Then based on your keywords, analyze each image content.
...
def eval_llm(chunk_score,info,frame_path,client):
    chunk_num=chunk_score.shape[0]
    pre_frame_path=os.path.dirname(frame_path)
    tmp_path=pre_frame_path+"/tmp"
    os.makedirs(tmp_path, exist_ok=True)
    image_path=[]
    gt_scores=chunk_score
    eval_frame_num=5
    final_score=[]
    last_res=""
    groups=create_groups(chunk_num,eval_frame_num)
    ...
    print("LLM score:",final_score,"GT score:",gt_scores)
    ...
    print(f"(PLCC): {plcc}, p-value: {plcc_p_value}, Index PLCC: {plcc_idx}, p-value: {plcc_idx_p_value}")
    print(f"(SRCC): {srcc}, p-value: {srcc_p_value}, Index SRCC: {srcc_idx}, p-value: {srcc_idx_p_value}")
...
def main():
    ...
    parser.add_argument("-dataset",type=str, default="tvsum", help="Folder containing videos to process.")
    parser.add_argument("-chunk", type=float,default=2, help="Duration of each chunk in seconds.")
    ...
    mos_all=read_mos("./sqoe3/mos/data.csv")
    ...
    for (index, video) in enumerate(video_list):
        ...
        # video_name = "EE-bNr36nyA"
        # video_name = "PuppiesBath"
        video_path = os.path.join(video_folder, video_name, f"{video_name}.mp4")
        frame_path = os.path.join(video_folder, video_name, "frames")
        video_meta_path = os.path.join(video_folder, video_name)
        score_path=video_meta_path+"/score.npy"
        ...
        if (args.dataset=="sqoe3"):
            chunk_score=[2,2,3,4,3,3,1,1,1,1,2,1,2,2,2,2,3,3,4,5]
            chunk_score=np.array(chunk_score)
            if (video_name=="FCB"):
                info="football game"
            else: info=video_name
        else:
            chunk_score=np.load(score_path)
            info=all_score[video_name]["info"] 
            chunk_score=np.mean(chunk_score,axis=0)

        print(f"Processing {video_name} with title {info}")
        plcc,srcc,plcc_idx,srcc_idx,weight=eval_llm(chunk_score,info,frame_path,client)
        save_weight(os.path.join(video_meta_path,"llm_weight.npy"),weight)

        weight=load_weight(os.path.join(video_meta_path,"llm_weight.npy"))
        # weight=average_every_n_elements(weight,n=4)
        # weight=load_weight(os.path.join(video_meta_path,"opt_weight.npy"))
        # plcc_ori,srcc_ori,plcc_llm,srcc_llm=eval_qoe(weight,video_name,mos_all)
        # plcc_ori_all.append(plcc_ori)
        # srcc_ori_all.append(srcc_ori)
        # plcc_llm_all.append(plcc_llm)
        # srcc_llm_all.append(srcc_llm)
        # save_weight(os.path.join(video_meta_path,"llm_weight.npy"),opt_weight)
        # print("average plcc:",np.mean(plcc),"average srcc:",np.mean(srcc),"average plcc_idx:",np.mean(plcc_idx),"average srcc_idx:",np.mean(srcc_idx))
        # if (index>=3): break
    # plcc_ori_all=np.array(plcc_ori_all)
    # srcc_ori_all=np.array(srcc_ori_all)
    # plcc_llm_all=np.array(plcc_llm_all)
    # srcc_llm_all=np.array(srcc_llm_all)
    # plot_cdf(plcc_ori_all,plcc_llm_all,"LLM","PLCC")
    # plot_cdf(srcc_ori_all,srcc_llm_all,"LLM","SRCC")
    # plot_cdf(plcc_ori_all,plcc_llm_all,"Opt","PLCC")
    # plot_cdf(srcc_ori_all,srcc_llm_all,"Opt","SRCC")
